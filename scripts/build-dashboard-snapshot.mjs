...
const fpdRows=readCsv(fpdInput).map(row=>({...row,credit_pass_date:row.loan_date})).sort((a,b)=>String(a.loan_date).localeCompare(String(b.loan_date)));
...
const snapshot={surface:'dashboard',title:'授信后提现与FPD监控',generatedAt:new Date().toISOString(),status:'reviewed',filters:[
  ...
  {id:'media',label:'媒体渠道',field:'media',multiple:true,defaultValue:[]}],queries:{
